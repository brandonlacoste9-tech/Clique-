// Upload API - Presigned URLs for media
  ...
  fastify.post('/story', async (request, reply) => {
    ...
    try {
      ...
      const result = await request.server.db.query(
        ...
        location 
          ? [userId, mediaKey, mediaType, thumbnailKey, caption, mood, isPublic, allowReplies, location.lng, location.lat, expiresAt]
          : [userId, mediaKey, mediaType, thumbnailKey, caption, mood, isPublic, allowReplies, expiresAt]
      );
